...
import com.emergence.study_app.Adapter.QuestionAdapter;
...
public class QuestionActivity extends BaseActivity implements MVPClass{
    TextView tv_timer;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        preferencesManager=new PreferencesManager(QuestionActivity.this);
    ...
    private void AddRecyclerJump() {
        ...
        Call<ResponseJumpQuestion> call=apiServices.JumptoQuestion("GetQuestionsList",preferencesManager.getUserId(),test_Id);
        call.enqueue(new Callback<ResponseJumpQuestion>() {
            ...
            public void onResponse(Call<ResponseJumpQuestion> call, Response<ResponseJumpQuestion> response) {
                ...
                if (response.body().getRes().equalsIgnoreCase("success")){
                    ...
                    recycler_answer.setAdapter(new JumpAdapter(getApplicationContext(),response.body().getData(),QuestionActivity.this));
                }else {
                }
            }
        ...
        new CountDownTimer(millisInFuture, countDownInterval) {
            ...
            public void onFinish() {
                if (isCompleted==false){
                    ShowDailoge(QuestionActivity.this);
                }
    ...
    private void AddQuestion(){
        showLoading();
        ApiServices apiServices= API_Config.getApiClient_ByPost();
        Call<ResponseQuestion> call=apiServices.getQuestion("GetQuestions",test_Id,preferencesManager.getUserId());
        call.enqueue(new Callback<ResponseQuestion>() {
            ...
            public void onResponse(Call<ResponseQuestion> call, Response<ResponseQuestion> response) {
                ...
                if (response.body().getRes().equalsIgnoreCase("success")){
                    ...
                    question_viewpager.setAdapter(new QuestionAdapter(getApplicationContext(),response.body().getData(),QuestionActivity.this));
    ...
    private void getanswer(String pos, String ques_id, String answer) {
        showLoading();
        ApiServices apiServices=API_Config.getApiClient_ByPost();
        Call<ResponseAnswer> call=apiServices.getAnswer("Answer",preferencesManager.getUserId(),test_Id,ques_id,ans,Quest_status);
...
//                      finish();
    ...
    public void fiishTest(){
        showLoading();
        ApiServices apiServices=API_Config.getApiClient_ByPost();
        Call<ResponseFinishTestSeriese> call=apiServices.finishTestSeriese("FinishTest",preferencesManager.getUserId(),test_Id);
            ...
            public void onResponse(Call<ResponseFinishTestSeriese> call, Response<ResponseFinishTestSeriese> response) {
                ...
                if (response.body().getRes().equalsIgnoreCase("success")){
                    ...
                    if (last_Index.equalsIgnoreCase("last")){
                        ...
                        Intent intent=new Intent(getApplicationContext(),CompletedAnswer.class);
