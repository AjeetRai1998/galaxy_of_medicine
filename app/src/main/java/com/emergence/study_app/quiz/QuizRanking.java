...
public class QuizRanking extends BaseActivity {
    ImageView iv_back;
    ...
    private void AddExamRecycler() {
        ...
        Call<ResponseRank> call=apiServices.getQuizRank("GetRank",id);
        call.enqueue(new Callback<ResponseRank>() {
            ...
            public void onResponse(Call<ResponseRank> call, Response<ResponseRank> response) {
                ...
                if (response.body().getRes().equalsIgnoreCase("success")){
                    ...
                    recycler_ranking.setAdapter(new QuizRankAdapter(getApplicationContext(),response.body().getData()));
                }else {
